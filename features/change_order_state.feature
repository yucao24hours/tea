...
フィーチャ:  管理者が、注文の管理（発注、受領、引換の登録）をできるようにしたい
  # XXX それぞれのシナリオで「今はいつか」「いつになっていることを前提としているか」がわからないので、直したい。
  ...
  シナリオ: 管理者として、発注する商品と、請求先ユーザーがわかる
    ...
    # XXX foo が yyy 円のお茶 hoge を n こ注文している、のステップでは、ステップ内でログアウトをするようになっているので対称性がとれていない。下手すると勘違いしてハマる可能性がある。
